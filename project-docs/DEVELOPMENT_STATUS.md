...
## Phase 5C final security and release audit

Audit date: 2026-09-07. Branch: `phase-5c-final-hardening`; initial working tree clean. Scope: approved REST/UI specifications, current configuration, controllers/DTOs/services, migrations, frontend auth/API/routes, tracked files and local artifacts. No features, endpoints, schema changes, dependencies or UI redesign were introduced. The following current findings supersede older milestone claims where they differ.

- **Authentication and authorization:** public registration/login, catalogue/categories and ACTIVE-product image reads are intentional. CUSTOMER route rules and server-side ownership checks cover account/address/cart/checkout/orders/custom requests/quotations/payments/shipments. All ADMIN routes remain centrally role-protected. Custom image reads require owner or ADMIN and return non-disclosing 404 for other customers. Empty Bearer tokens now return normalized 401 instead of escaping the filter as an argument error. JWT exception text is no longer logged.
- **Secrets and configuration:** no real committed secret was found by current tracked-text/DOCX pattern inspection or comparison against sensitive values in the ignored local `.env`. No developer home-directory paths or real access-token/private-key pattern candidates were found. This is not a full Git-history secret audit. The committed development JWT signing-key fallback was a security defect: removed, together with the default database password. `JWT_SECRET` and `DB_PASSWORD` must now be supplied to the backend process. `.env.example` contains only safe placeholders and a blank JWT key; actual `.env` is ignored and unchanged. Spring Boot does not automatically load the root `.env`. No secret was generated. The development seeder remains opt-in and requires explicitly supplied credentials.
- **Input validation:** representative auth, address, category/product, inventory, cart/checkout, payment, custom request, quotation and shipment DTO/controller/service checks reviewed. Negative price/stock and nonpositive cart quantities are rejected. Fixed integer overflow when accumulating cart quantities. DEC-005 was not actually enforced by the old quotation backend: it accepted absent/zero/oversized advances despite frontend validation. Creation now requires `0 < advanceAmount <= quotedAmount`, and payment rejects invalid legacy stored advances before saving. Existing optional shipment carrier/tracking fields remain optional; no new business rules were invented.
- **Uploads:** both upload roots are normalized absolute paths, filenames are generated UUIDs, size metadata comes from the multipart file, and logical references/content URLs exclude physical roots. Read paths reject absolute/traversing references and real-path escapes. Upload writes use controlled generated destinations. Product reads require an ACTIVE parent; reference reads enforce owner/ADMIN and no-store. Configurable multipart defaults remain 10MB/file and 12MB/request, with existing normalized 413 coverage. Broad `image/*` MIME admission also accepts active formats; both content endpoints now send `Content-Security-Policy: sandbox; default-src 'none'` alongside Spring Security nosniff protection. Header behavior is covered by MockMvc; no new browser execution test was performed. MIME sniffing/decoding and the final business allowlist remain outside the accepted MVP policy (DEC-003).
- **Payments:** FULL uses stored order total, ADVANCE uses stored quotation advance. No dedicated card/CVV/PIN/provider-credential fields are accepted or stored; payment method is a non-sensitive label. Sandbox success and SANDBOX references remain explicit. Duplicate/state checks were vulnerable to simultaneous submissions; payment initiation now locks the owned parent row before reading payable state and holds the lock through the transaction. Existing sequential success/ownership/duplicate regressions pass; live concurrent PostgreSQL payment execution was not performed in this phase. External callback remains unimplemented under DEC-001.
- **Orders/inventory/workflow:** checkout still calculates prices and totals on the server, revalidates inventory under existing pessimistic locks, saves item/address snapshots, decrements stock and clears cart in one transaction. Database checks also reject negative stock/invalid quantities. No tax/delivery formula or reservation architecture was added. Fixed custom shipment status updates that could bypass production or rewind the request: they now validate the parent's approved transition before mutation, permitting alignment when the separate parent endpoint already reached the same state.
- **Errors:** existing validation/auth/ownership/not-found/workflow/oversize and safe unexpected-error envelopes remain. Added normalized 400 handling for malformed JSON, invalid typed parameters and missing required request parts/parameters; database constraint failures now return a generic 409 without SQL or constraint details. Unexpected errors expose a generic message. New tests cover malformed input, database-information suppression and safe unexpected-error responses.
- **CORS/HTTP security:** only `http://localhost:5173` is allowed, credentials are disabled, methods are bounded to GET/POST/PUT/PATCH/DELETE/OPTIONS and headers to Authorization/Content-Type. Existing JWT filter ordering and stateless configuration remain. No deployment domain was assumed.
- **Flyway/database:** V1-V5 and their dependency order reviewed. Flyway is enabled, `ddl-auto=none`, baseline-on-migrate and out-of-order are false. Migrations define the domain schema without mandatory manually inserted data; development seeding is optional. Empty-database reconstruction was deliberately not executed. The default Maven suite excludes tagged PostgreSQL integration tests, so it is not evidence of a fresh database rebuild or live locking test. Existing V5 allows legacy invalid advances; application checks now prevent their payment without rewriting applied migrations.
- **Repository/frontend:** actual environments, Maven/Vite outputs, node_modules, uploads, logs, temporary and coverage artifacts found are ignored. No tracked generated/runtime artifact requiring removal was found; `.gitignore` needs no change. Frontend API URL uses `VITE_API_BASE_URL` with the intentional localhost fallback; shared token handling/401 cleanup and auth/role guards remain intact. No token console logging, developer filesystem paths or debug-only visible controls found. Existing page loading/error states remain. Frontend implementation unchanged.
- **Decisions:** DEC-002 stays OPEN with client-side logout accepted for MVP; DEC-003 and DEC-006 are accurately DEFERRED for final upload policy and cancellation. DEC-001/004/007/010/012 remain DEFERRED (provider, requotation, charges, automatic default-address behavior, browser E2E). DEC-005/008/009/011/013 remain APPROVED. No blocked capability is represented as implemented.

Verification: full `mvn clean test` reports **402 tests, 0 failures, 0 errors, 0 skipped; BUILD SUCCESS** (21 additional cases). Initial sandbox Maven access failed; an affected-suite run encountered stale compiled classes, then a clean compilation identified a missing regression-test import which was corrected. The final clean suite passed. PowerShell labelled a JVM class-sharing warning as a native-command error; Maven and all Surefire reports show success. Frontend `npm run test:run -- --pool=threads --maxWorkers=1`: **65 tests across 9 files passed**, exit 0, in CMD with no concurrent builds. Two earlier attempts timed out starting the navigation worker (53 tests completed each); they are not counted as passing runs. `npm run build`: PASS, 142 modules. `npm run lint`: PASS, exit 0 with no warnings/errors. `git diff --check`: PASS. No test-runner configuration or isolation setting was changed.

Release/demo prerequisites and next task: supply an independently chosen Base64 JWT signing key (at least 32 decoded bytes) and database password in the process environment; do not reuse the old public development key. The local root `.env` currently supplies no JWT key and was not changed. Perform the separately authorized empty-database reconstruction next, including startup/Flyway and focused payment-concurrency checks, then README/demo preparation. Prior live standard/custom workflow checks remain historical evidence; manual/responsive acceptance is supplied by the user's Phase 5C request, not newly executed here. No database reset, public README edit, staging, commit, branch switch or other prohibited Git operation was performed.

Phase 5C modified files (25, all existing files; no new tracked files):

- `.env.example`
- `backend/src/main/java/com/handmadeart/ecommerce/controller/CustomOrderImageController.java`
- `backend/src/main/java/com/handmadeart/ecommerce/controller/ProductImageController.java`
...
- `backend/src/test/java/com/handmadeart/ecommerce/CustomArtworkPhase2ServiceTest.java`
- `backend/src/test/java/com/handmadeart/ecommerce/CustomArtworkServiceTest.java`
- `backend/src/test/java/com/handmadeart/ecommerce/GlobalExceptionHandlerTest.java`
- `backend/src/test/java/com/handmadeart/ecommerce/PaymentServiceTest.java`
- `backend/src/test/java/com/handmadeart/ecommerce/SecurityAuthorizationTest.java`
- `project-docs/DECISION_LOG.md`
- `project-docs/DEVELOPMENT_STATUS.md`
- `project-docs/FRONTEND_DEVELOPMENT_STATUS.md`
