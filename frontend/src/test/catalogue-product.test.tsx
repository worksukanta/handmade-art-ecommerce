...
describe('catalogue search regression', () => {
  ...
  it('dismisses filters outside and on Escape while preserving panel interactions', async () => {
    renderWithRouter(<CataloguePage />)
    await screen.findByText('No products match your filters')
    const toggle = screen.getByRole('button', { name: 'Filters' })
    await userEvent.click(toggle)
    await userEvent.click(screen.getByLabelText('Minimum price'))
    expect(toggle).toHaveAttribute('aria-expanded', 'true')
    await userEvent.selectOptions(screen.getByLabelText('Category'), '')
    expect(toggle).toHaveAttribute('aria-expanded', 'true')
    await userEvent.click(screen.getByRole('heading', { name: 'Explore the catalogue' }))
    expect(toggle).toHaveAttribute('aria-expanded', 'false')
    await userEvent.click(toggle)
    await userEvent.click(screen.getByLabelText('Minimum price'))
