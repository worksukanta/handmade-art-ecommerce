...
export function AdminProductsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setData(await adminCommerceService.listProducts(page))
      setError(null)
    } catch (e) {
    ...
    <section className="commerce-page">
      <div className="heading-actions">
        <div className="page-heading">
          ...
          <h1>Products</h1>
          <p>Manage all active and inactive catalogue records.</p>
        </div>
      ...
      {loading && !data && !error ? (
        <LoadingState label="Loading products" cards />
      ) : error ? (
        <ErrorState title="We couldn't load products" message={error} onRetry={() => void load()} />
      ) : !data?.content.length ? (
        <EmptyState title="No products" message="Create the first catalogue product." />
      ) : (
        <>
              ...
              <tbody>
                {data.content.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{humanizeStatus(p.product_type)}</td>
                    <td>{p.category_name}</td>
                    <td>{formatCurrency(p.price)}</td>
                    <td>
                      <StatusBadge kind="generic" value={p.status} />
                    </td>
                    <td>
                      <Link to={`/admin/products/${p.id}`}>Manage</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {data.total_pages > 1 && (
            <nav className="pagination" aria-label="Admin products pagination">
